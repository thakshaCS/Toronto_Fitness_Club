...
function SubscriptionsChoose() {
    ...
    function handleClick() {
        ...
        fetch('http://localhost:8000/subscriptions/choose/', {
          method: 'POST',
          body: dataa,
          credentials: 'include',
        })
        ...
        .then(() => {
            ...
            if (obj['message'] == "Subscription plan successfully chosen") {
                Cookies.set('pk', obj['data']['pk'])
                window.location.href = '/subscriptions/card';
            }
        });
    ...
    fetch ('http://localhost:8000/subscriptions/available/', {
        credentials: 'include',
    })
    .then ((response) => response.json())
    .then(data => obj = data)
    ...
    .then(() => {
        ...
        for (var i=0; i < obj['data'].length; i++) {
            ...
            length = obj['data'][i]['length'];
            
            table.innerHTML += `<tr>
                <td>
                    <div className='d-flex'>
                    <div className='ms-3'>
                        <h6  className='fw-bold h6'>` + name + `</h6>
                    </div>
                    </div>
                </td>
                <td>
                    <h6 className='fw-normal mb-1 h6'>` + price + `</h6>
                </td>
                <td>
                    <h6 className='fw-normal mb-1 h6'>` + length + `</h6>
                </td>
                </tr>`
        }
        if (obj['data']['message'] == "Subscription plan successfully chosen") {
            window.location.href = '/subscriptions/card';
        } 
    });
    ...
    return (
        <MDBContainer style={{marginLeft: '22%', marginRight: '30%', marginTop: '10%', width: '150%'}}>
            ...
            <MDBCard  className='text-black m-5' style={{marginLeft: '1000px', borderRadius: '25px', width: '70%'}}>
                <MDBCardBody>
                <MDBRow>
                        <p className="text-center h1 fw-bold mb-5 mx-1 mx-md-4 mt-4">Available Subscription Plans</p>

                        <MDBTable align='middle'>
                        <MDBTableHead>
                            <tr >
                            <th className='fw-bold h5' scope='col'>
            ...
            <MDBCard className='text-black m-5' style={{marginLeft: '1000px', borderRadius: '25px', width: '70%'}}>
                ...
                <MDBRow>
                    <MDBCol md='10' lg='6' className='order-2 order-lg-1 d-flex flex-column align-items-center'>
                        ...
                        <p className="text-center h1 fw-bold mb-5 mx-1 mx-md-4 mt-4">Choose Plan</p>
                        ...
                        <MDBCardImage src='https://cdn-icons-png.flaticon.com/512/2098/2098402.png' fluid/>
                    </MDBCol>
                </MDBRow>
                </MDBCardBody>
            </MDBCard>
...
export default SubscriptionsChoose;